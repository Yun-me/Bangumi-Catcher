...
> 结构性重构版本：核心逻辑与 UI 分层、修复多处体验/正确性问题、新增标签统计与收藏筛选。

### 新增 (Added)

- **项目分层重构**：拆分为 `bangumi_catcher/core`（API、分析、缓存、配置、模型、导出）与 `bangumi_catcher/ui`（PySide6 界面、主题、图表），职责更清晰，便于维护与测试。
- **热门标签统计**：分析报告新增 `tag_counts` / `top_tags`，GUI 与 HTML 报告新增「热门标签 Top 15」图表。
- **收藏状态筛选**：收藏总表支持按「想看 / 看过 / 在看 / 搁置 / 抛弃」快速过滤，可与搜索组合使用。
- **收藏表格右键菜单**：支持在浏览器打开条目、复制条目链接。
- **报告 JSON 导出**：文件菜单新增「导出报告 JSON…」，便于二次处理分析结果。
- **缓存管理交互**：清空缓存前显示条目数量并二次确认，避免误清。

### 修复 (Fixed)

- **matplotlib 后端被覆盖**：`visualizer.py` 无条件调用 `matplotlib.use("Agg")`，会覆盖 GUI 已初始化的 Qt 后端；现仅在非 Qt 后端时切换到 Agg。
- **HTML 报告图表缓存串数据**：导出 HTML 后 `_charts_uri` 未随新报告重置，导致抓取新用户后仍导出旧图表；现每次抓取完成都会清空该缓存。
- **取消响应不及时**：抓取过程新增 `cancel_check` 快速中断机制，在每次请求前后、分页与补全循环中检查取消状态，不再只能等到下一次进度回调。
- **模板路径失效**：重构后 `gui.py` 的 HTML 模板路径未同步更新，现指向 `bangumi_catcher/templates`。
- **配置年份硬编码**：`analysis.year_end` 默认值改为当前年份，避免年份过期导致新收藏不进入年度统计。

### 变更 (Changed)

- 顶部栏改为自适应布局：输入框可伸缩、控件最小宽度更友好，小窗口下不再挤成一团。
- `APP_VERSION` 统一从包版本读取，避免多处版本号漂移。
- `README.md` 更新为新的项目结构与功能说明。

### 性能 (Performance)

- 分析引擎的「最爱排行」与「评分对比」改用 `heapq.nlargest` 取 Top N，避免全量排序。
- 图表总数由 8 增至 9，HTML 导出仍为按需生成。

### 测试 (Tests)

- 更新测试导入路径以匹配新分层结构。
- 新增标签统计断言，并同步图表数量断言（8 → 9）。
